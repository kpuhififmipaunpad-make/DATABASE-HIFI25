...
function initDataTable() {
  ...
  const dt = $table.DataTable({
    ...
    lengthChange: true,  // ✅ ENABLE dropdown items per page
    searching: true,     // ✅ ENABLE search box
    pagingType: isMobile ? 'simple' : 'simple_numbers',
    pageLength: defaultPageLength,
    
    // ✅ DOM Layout yang benar
    ...
    columnDefs: [
      { targets: [0], width: isMobile ? '40px' : '48px', className: 'text-center' },
      { targets: [8], orderable: false, className: 'no-detail text-center' }
    ],
